...
const services = [
  {
    ...
    description: "Comprehensive forest management for long-term ecological balance.",
  },
  {
    icon: <LeafyGreen size={32} strokeWidth={1.5} className="text-[#78B943]" />,
    hoverIcon: <LeafyGreen size={32} strokeWidth={1.5} className="text-black" />,
  ...
  {
    icon: <MountainSnow size={32} strokeWidth={1.5} className="text-[#78B943]" />,
    hoverIcon: <MountainSnow size={32} strokeWidth={1.5} className="text-black" />,
...
const ServicesSection = () => {
  ...
  const visibleServices = services.slice(page * CARDS_PER_PAGE, (page + 1) * CARDS_PER_PAGE);
...
>
  <div
    ...
    style={{
      pointerEvents: "none",
      background: "linear-gradient(to top right, rgba(255,255,255,0.8) 0%, rgba(255,255,255,1) 50%, rgba(255,255,255,1) 100%)"
    }}
  />
          ...
          {visibleServices.map((service, i) => (
            ...
            >
              <div className="mb-6 p-4 rounded-full bg-[#EAFDD5] group-hover:bg-[#78B943] transition-colors duration-300">
                ...
                <span className="hidden group-hover:block">{service.hoverIcon}</span>
              </div>
        ...
        <div className="flex justify-center gap-2 mt-12">
          {Array.from({ length: totalPages }).map((_, idx) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${page === idx ? 'bg-[#FFA726] w-6' : 'bg-[#EAEAEA] hover:bg-[#FFA726]/50'}`}
