...
export default function FlightSearch() {
  ...
  const [currentStep, setCurrentStep] = useState('outbound'); // outbound or inbound
  ...
  const outboundFlights = generateFlights(origin, destination, departDate, false);
  const inboundFlights = tripType === 'roundTrip' ? generateFlights(origin, destination, returnDate, true) : [];
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
                  {currentStep === 'outbound' ? (
                    <>
                      {origin} <ArrowRight className="h-6 w-6 text-blue-600" /> {destination}
                    </>
                  ) : (
                    <>
                      {destination} <ArrowRight className="h-6 w-6 text-blue-600" /> {origin}
                    </>
                  )}
                </h1>
      ...
      <div className="container mx-auto px-4 py-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-semibold">
                  {currentStep === 'outbound' ? 'Choose outbound flight' : 'Outbound flight selected'}
                </div>
                ...
                {selectedFlights.outbound && (
                  <div className="text-sm opacity-90 mt-1">
                    {selectedFlights.outbound.flight.flightNumber} • ${selectedFlights.outbound.price}
                  </div>
                )}
              </div>
            </div>
          ...
          {tripType === 'roundTrip' && (
            <Card className={`flex-1 p-6 transition-all duration-300 ${
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-semibold">
                    {currentStep === 'inbound' ? 'Choose return flight' : selectedFlights.inbound ? 'Return flight selected' : 'Return flight'}
                  </div>
                  <div className="text-sm opacity-90">
                    {destination} → {origin}
                  </div>
                  {selectedFlights.inbound && (
                    <div className="text-sm opacity-90 mt-1">
                      {selectedFlights.inbound.flight.flightNumber} • ${selectedFlights.inbound.price}
                    </div>
                  )}
                </div>
              </div>
            ...
            return (
              <Button
                ...
                className={`min-w-[200px] flex-col h-auto py-4 transition-all duration-300 ${
                  isSelected ? "bg-blue-600 hover:bg-blue-700 shadow-lg" : "hover:shadow-md"
                }`}
              >
        ...
        <div className="space-y-6">
          {getCurrentFlights().map((flight) => (
            <Card key={flight.id} className="overflow-hidden hover:shadow-xl transition-all duration-300 bg-white">
              <div className="p-8">
                <div className="flex items-center justify-between">
                  {/* Flight Info */}
                  <div className="flex-1">
                    <div className="flex items-center gap-8 mb-6">
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-sm text-gray-600 font-medium">{flight.departure.airport}</div>
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-sm text-gray-600 font-medium">{flight.arrival.airport}</div>
                    ...
                    <div className="flex items-center gap-6 text-sm text-gray-600">
                      ...
                      <span className="px-2 py-1 bg-gray-100 rounded-md font-medium">{flight.flightNumber}</span>
                      ...
                      >
                        ...
                        {hoveredEmission === flight.id && (
                          <div className="absolute bottom-full left-0 mb-2 p-4 bg-gray-900 text-white text-xs rounded-lg shadow-xl w-72 z-10">
                            <div className="font-medium mb-2">Carbon Emission Details</div>
                            <div>This flight will emit approximately {flight.carbonEmission} kg of CO₂ per passenger.</div>
                            <div className="mt-2 text-green-400 font-medium">
                              Offset available for ${Math.round(flight.carbonEmission * 0.02)}
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex gap-4 ml-8">
                    <Card className="p-6 text-center min-w-[140px] hover:shadow-lg transition-all duration-300 cursor-pointer border-2 hover:border-blue-200">
                      <div className="text-xs text-gray-600 font-semibold mb-2">ECONOMY</div>
                    ...
                    <Card className="p-6 text-center min-w-[140px] hover:shadow-lg transition-all duration-300 cursor-pointer border-2 hover:border-purple-200">
                      <div className="text-xs text-gray-600 font-semibold mb-2">BUSINESS</div>
